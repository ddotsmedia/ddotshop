...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      // Hydrate role once (SUPER_ADMIN users have no tenant/shop).
      if (token.uid && !token.role) {
        const u = await prisma.user.findUnique({
          where: { id: token.uid as string },
          select: { role: true },
        });
        token.role = u?.role ?? "TENANT";
      }
      // (Re)hydrate tenant/shop context when missing (shop appears after onboarding).
      if (token.uid && token.role !== "SUPER_ADMIN" && (!token.tenantId || !token.shopId)) {
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.role = (token.role as string | undefined) ?? "TENANT";
